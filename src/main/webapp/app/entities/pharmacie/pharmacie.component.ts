...
export class PharmacieComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.pharmacieService
      ...
      .subscribe(
        (res: HttpResponse<IPharmacie[]>) => this.paginatePharmacies(res.body, res.headers),
  ...
  trackId(index: number, item: IPharmacie) {
    return item.id;
  }
  ...
  protected paginatePharmacies(data: IPharmacie[], headers: HttpHeaders) {
